...
class FEATURE_ENGINEERING:
    ...
    def _compute_lag(self, df: pl.LazyFrame) -> pl.LazyFrame:
        ...
        return (
            df.with_columns(
                [pl.col(col).shift(window).alias(f"{col}_lag_{window}") for col in names for window in [5, 10, 90, 252] if any([f"{col}_lag_{window}" in impt_col for impt_col in CONFIG.FEATURES])]
            )
        ...
        return (
            df.with_columns(
                [
                    ...
                    if any([f"{col}_return_skew_{window}" in impt_col for impt_col in CONFIG.FEATURES])
                ]
            )
            .drop([i for i in df.collect_schema().names() if i != CONFIG.DATE_COL])
            .fill_null(0)
        ...
        return (
            df.with_columns(
                [
                    ...
                    for window in windows
                    if (
                        any([f"{col}_sma_{window}" in impt_col for impt_col in CONFIG.FEATURES])
                        or any([f"{col}_{window}_sharpe" in impt_col for impt_col in CONFIG.FEATURES])
                    )
                ]
                + [
                    ...
                    if col
                    for window in windows
                    if any([f"{col}_vol_{window}" in impt_col for impt_col in CONFIG.FEATURES])
                ]
                + [
                    (
                    ...
                    if col
                    for window in windows
                    if any([f"{col}_{window}_sortino" in impt_col for impt_col in CONFIG.FEATURES])
                ]
            )
            .with_columns(
                [
                    ...
                    for col in names
                    for window in windows
                    if any([f"{col}_{window}_sharpe" in impt_col for impt_col in CONFIG.FEATURES])
                ]
            )
            .drop(names)
        )
    ...
    def _compute_pairs_features(self, df: pl.LazyFrame) -> pl.LazyFrame:
        ...
        for pair1, pair2 in itertools.combinations(names, 2):
            ...
            prefix = f"{pair1}_{pair2}"
            for op_name, expr in operations:
                feature_name = f"{prefix}_{op_name}"
                if any([feature_name in impt_col for impt_col in CONFIG.FEATURES]):
                    exprs.append(expr.alias(feature_name))
