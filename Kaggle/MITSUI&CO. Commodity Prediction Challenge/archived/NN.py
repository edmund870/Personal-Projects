...
class NN:
    ...
    **kwargs : dict
    ...
    def __init__(
        ...
        huber_weight: float = 30.0,
        MSE_weight: float = 10.0,
        ranking_weight: float = 10.0,
        ...
        **kwargs,
        ...
        self.criterion = CombinedICIRLoss(huber_weight=huber_weight, mse_weight=MSE_weight, ranking_weight=ranking_weight)
        self.val_criterion = CombinedICIRLoss(huber_weight=huber_weight, mse_weight=MSE_weight, ranking_weight=ranking_weight)
        self.update_criterion = CombinedICIRLoss(huber_weight=huber_weight, mse_weight=MSE_weight, ranking_weight=ranking_weight)
    ...
    def fit(self, train_set: tuple, val_set: tuple, retrain_set: tuple, verbose: bool = False) -> None:
        ...
        if verbose:
            ...
            print(
                f"{'Epoch':^5} | {'Train Loss':^10} | {'Train ICIR Loss':^15} | {'Train MSE Loss':^14} | {'Train Huber Loss':^17} | {'Train Ranking Loss':^17} | {'Val Loss':^8} | {'Val ICIR Loss':^13} | {'Val MSE Loss':^12} | {'Val Huber Loss':^16} | {'Val Ranking Loss':^16} | {'Train sharpe':^9} | {'Val sharpe':^7} | {'LR':^7}"
            )
            print("-" * 60)
        ...
        for epoch in range(self.epochs):
            train_loss, train_sharpe, train_icir_loss, train_mse_loss, train_huber_loss, train_ranking_loss = self.train_one_epoch(
                seq_train_dataloader, verbose
            )
            val_loss, val_sharpe, val_icir_loss, val_mse_loss, val_huber_loss, val_ranking_loss = self.validate_one_epoch(
            ...
            if verbose:
                print(
                    f"{epoch + 1:^5} | {train_loss:^10.4f} | {train_icir_loss:^15.4f} | {train_mse_loss:^14.4f} | {train_huber_loss:^17.4f} | {train_ranking_loss:^17.4f} | {val_loss:^8.4f} | {val_icir_loss:^13.4f} | {val_mse_loss:^12.4f} | {val_huber_loss:^16.4f} | {val_ranking_loss:^16.4f} | {train_sharpe:^9.4f} | {val_sharpe:^7.4f} | {lr_last:^7.5f}"
                )
    ...
    def train_one_epoch(self, seq_train_dataloader: DataLoader, verbose: bool) -> tuple:
        ...
        for seq_batch in seq_train_dataloader:
            ...
            with torch.autocast(device_type="cuda"):
                ...
                loss, icir_loss, mse_loss, huber_loss, ranking_loss, _ = self.criterion(pred_y, true_y).values()
        ...
        train_mse_loss = total_mse_loss / len_data_loader
        train_huber_loss = total_huber_loss / len_data_loader
        train_ranking_loss = total_ranking_loss / len_data_loader

        return train_loss, train_sharpe, train_icir_loss, train_mse_loss, train_huber_loss, train_ranking_loss
    ...
    def validate_one_epoch(self, seq_val_dataloader: DataLoader, retrain_val_dataloader: DataLoader, verbose=False) -> tuple:
        ...
        losses, icir_losses, mse_losses, huber_losses, ranking_losses = [], [], [], [], []
        all_y, all_preds = [], []
        ...
        for seq_batch, retrain_batch in zip(seq_val_dataloader, retrain_val_dataloader):
            ...
            if self.refit:
                ...
                loss = self.val_criterion(pred_y, retrain_true_y)["total_loss"]
                loss.backward()
            ...
            with torch.inference_mode():
                ...
                loss, icir_loss, mse_loss, huber_loss, ranking_loss, _ = self.val_criterion(pred_y, true_y).values()
                losses.append(loss.cpu().numpy())
                icir_losses.append(icir_loss.cpu().numpy())
                mse_losses.append(mse_loss.cpu().numpy())
                huber_losses.append(huber_loss.cpu().numpy())
        ...
        val_huber_loss = np.mean(huber_losses)
        val_ranking_loss = np.mean(ranking_losses)

        sharpe = self.rank_correlation_sharpe(all_y, all_preds)

        return loss, sharpe, val_icir_loss, val_mse_loss, val_huber_loss, val_ranking_loss
    ...
    def update(self, seq_X: np.array, true_y: np.array):
        ...
        with torch.autocast(device_type="cuda"):
            ...
            loss = self.update_criterion(pred_y, true_y)["total_loss"]
