...
class BaseFinancialDataset(Dataset, ABC):
    ...
    def _prepare_samples(self):
        ...
        self.X = torch.tensor(self.X.drop(CONFIG.DATE_COL).to_numpy(), dtype=torch.float32, device=self.device)
        self.y = torch.tensor(self.y.drop(CONFIG.DATE_COL).to_numpy(), dtype=torch.float32, device=self.device)
...
class SequentialDataset(BaseFinancialDataset):
    ...
    def __init__(self, X: pl.DataFrame, Y: pl.DataFrame, seq_len: int = CONFIG.SEQ_LEN, date_column: str = CONFIG.DATE_COL):
        """
