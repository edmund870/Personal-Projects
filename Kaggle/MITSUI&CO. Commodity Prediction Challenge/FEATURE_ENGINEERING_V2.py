...
class FEATURE_ENGINEERING:
    ...
    def _compute_pairs_features(self, df: pl.LazyFrame) -> pl.LazyFrame:
        ...
        for pair1, pair2 in CONFIG.PAIRS:
            p1 = pl.col(f"{pair1}_log_ret")
            p2 = pl.col(f"{pair2}_log_ret")
            for prefix in [f"{pair1}_log_ret_{pair2}_log_ret", f"ranked_{pair1}_log_ret_{pair2}_log_ret"]:
                key = "pairs" if prefix == f"{pair1}_log_ret_{pair2}_log_ret" else "ranked_pairs"
                exprs[key].extend(
                    [
                        # Arithmetic
                        (p1 + p2).alias(f"{prefix}_add"),
                        (p1 - p2).alias(f"{prefix}_sub"),
                        (p1 * p2).alias(f"{prefix}_mul"),
                        pl.when(p2 != 0).then(p1 / p2).otherwise(None).alias(f"{prefix}_div"),
                        # Polynomial
                        ((p1 + p2) ** 2).alias(f"{prefix}_poly2"),
                        ((p1 + p2) ** 3).alias(f"{prefix}_poly3"),
                        ((p1 - p2) ** 2).alias(f"{prefix}_diff_squared"),
                        # Nonlinear transforms
                        (p1 * p2).sqrt().alias(f"{prefix}_sqrt_mul"),
                        (1 + (p1 * p2)).log().alias(f"{prefix}_log_mul"),
                        (p1 - p2).exp().alias(f"{prefix}_exp_diff"),
                        # Statistical / Comparative
                        p1.min().alias(f"{prefix}_min"),
                        p1.max().alias(f"{prefix}_max"),
                        ((p1 + p2) / 2).alias(f"{prefix}_mean"),
                        (p1 - p2).abs().alias(f"{prefix}_abs_diff"),
                        pl.when((p1 + p2) != 0).then((p1 - p2) / (p1 + p2 + err)).otherwise(None).alias(f"{prefix}_norm_diff"),
                        pl.when((p1 + p2) != 0).then(2 * p1 * p2 / (p1 + p2)).otherwise(None).alias(f"{prefix}_harmonic_mean"),
                        pl.when(p2 != 0).then((p1 / p2).arctan()).otherwise(None).alias(f"{prefix}_atan_ratio"),
                        pl.when(((p1 + err) / (p2 + err)) > 0).then(((p1 + err) / (p2 + err)).log()).otherwise(None).alias(f"{prefix}_log_ratio"),
                        # Logistic / sigmoid
                        (1 / (1 + (p1 - p2).neg().exp())).alias(f"{prefix}_sigmoid_diff"),
                        # Binary indicator
                        pl.when(p1 > p2).then(1).otherwise(0).cast(pl.Int8).alias(f"{prefix}_gt_flag"),
                    ]
                )
        return (
